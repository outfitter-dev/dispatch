    ...
    async def reserve_lane_launch(
        ...
        *,
        ...
        if native_id is not None:
            raise ValidationError("lane launch reservations require an unresolved provider thread")
        ref, ref_source, ref_payload, ref_mixer = await self._allocate_ref_parts(
            lane_id,
            provider=provider,
            binding_id=binding_id,
    ...
    async def claim_delivery(self, delivery_id: str) -> bool:
        ...
        async with self._transaction():
            cur = await self._conn.execute(
                ...
                "AND NOT EXISTS ("
                "SELECT 1 FROM lane_runtime_state runtime "
                "WHERE runtime.lane = deliveries.lane AND deliveries.provider = 'hermes' "
                "AND runtime.needs_attention = 1"
                ") "
                "AND NOT EXISTS ("
