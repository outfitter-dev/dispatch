...
releases, :data:`CHANGED_SINCE_PARENT` and :data:`ADDED_SINCE_PARENT` are edited
by hand. The baseline test requires every changed or new op to be classified so
it cannot be forwarded accidentally to a pre-handshake parent-release daemon.
...
ADDED_SINCE_PARENT: frozenset[str] = frozenset({"delivery-get", "delivery-reconcile"})
