...
| `search` (`search`) | experimental `thread/search` for broad search; registry `thread_items` for `--local`; `thread/read(includeTurns:true)` for one-thread search | Broad search uses App Server search plus dispatch-side managed/unmanaged, repo/directory, and date filters. `--local` searches Dispatch's normalized managed-history index without App Server search. Thread-focused search reads one transcript and scans locally because App Server search has no thread-id filter. |
