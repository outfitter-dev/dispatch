...
- Release publishing: bump `project.version` on a PR; a green `main` CI run cuts GitHub Release `v<version>` when that tag is missing, and `publish.yml` Trusted-Publishes to PyPI then confirms a clean `uvx` install.
