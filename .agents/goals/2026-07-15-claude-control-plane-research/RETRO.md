...
2026-07-15 - Global Agent View cockpit topology correction
- Production uses one shared unscoped `claude agents` cockpit per local Claude runtime. `--cwd` remains only a disposable-test or explicit operator filter; per-repository cockpit proliferation is rejected.
- Unscoped `claude agents --json --all` is the global identity roster. Cwd/worktree remains mutable routing metadata, preserving cross-repository visibility rather than defining cockpit ownership.
- Guarded selection joins provider-qualified full session UUID, current cwd/worktree, and exactly one visible row. Missing, duplicate, or ambiguous joins fail closed before input; the executable cockpit-plan fixture enforces this contract.
...
| global-cockpit topology fixture | focused fixture, shell, sdist, and package-content gates | passed | unscoped cockpit/global roster, provider-session-cwd-row join, and ambiguous-row abort enforced |
| global-cockpit full repository gate | `just check` | passed | Ruff, format, strict mypy, 696 passed/17 deselected, wheel/sdist and contents |
...
| DIS-54 | Backlog | Preferred single global unscoped zmx-hosted Agent View cockpit prerequisite; global roster and fail-closed target join specified; zmx transaction/redaction and aggregate receipt proof block DIS-50 enablement/acceptance |

DIS-54 comment `a37d74a2-5600-48ae-ba4e-6959ee11e0d8` records the global
cockpit topology and fail-closed routing acceptance amendment.
