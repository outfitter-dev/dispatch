...
async def test_first_delivery_is_reserved_and_linked_atomically(store: Registry) -> None:
    ...
    await store.update_lane_provider_thread(
        ...
        provider="hermes",
        binding_id="hermes-default",
        provider_thread_id="stored-after-compression",
    )
...
async def test_v28_migration_preserves_rows_and_adds_lane_launches(tmp_path: Path) -> None:
    db = tmp_path / "registry-v28.db"
    ...
    try:
        ...
        async with migrated._conn.execute("PRAGMA user_version") as cur:
            row = await cur.fetchone()
        assert row is not None and int(row[0]) == SCHEMA_VERSION == 29
